...
from scripts1.datas.bank.data_icbc_001_transferRstQry import *
...
def transferRstQry_data(test_host):
    """用例集"""
    if test_host == "icbc":
        Transfer_Data = TransferRstQry_icbc_Data()
    return Transfer_Data


import pytest
cases = []
for i in transferRstQry_data("icbc").cases:
    cases.append((i["title"],i["case"],i["expection"]))

@pytest.mark.parametrize("title,case,expection",cases)
def test_icbc_transferRstQry(test_host,title,case,expection):
    """工行-单笔付款结果查询"""
    title = title
...
if __name__ == "__main__":
    ...
    for i in range(len(TransferRstQry_Data.cases)):
        if i>=0:
            title = TransferRstQry_Data.cases[i]["title"]
            case =  TransferRstQry_Data.cases[i]["case"]
            resp = bank_req("SIT",case,api,method)
