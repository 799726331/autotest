...
from scripts1.datas.bank.data_abc_002_batchTransfer import *
...
def batchTransfer_data(test_host):
    """用例集"""
    if test_host == "abc":
        BatchTransfer_data = BatchTransfer_abc_data("abc").get_case()
    return BatchTransfer_data

import pytest
cases = []
test_list = batchTransfer_data("abc")
...
def test_abc_batchtransfer(test_host,title,case,expection):
    """农行-批量支付测试用例 """
    title = title
    ...
    for i in range(len(test_case)):
        if i in [6]:
            ...
            try:
                ...
                print("")
                with open("abc_batchtransfer.txt","a+",encoding="utf-8")as f:
                    f.write("农行-批量支付接口"+title + ":" + "\n")
