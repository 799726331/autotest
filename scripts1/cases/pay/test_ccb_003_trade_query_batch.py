...
from scripts1.datas.pay.data_ccb_003_trade_query_batch import *
...
def data_list(test_host):
    ...
    data = Trade_query_batch_ccb_data()
    return data
...
if __name__ == "__main__":
    ...
    for i in range(len(Trade_query_batch_ccb_data.cases)):
        print("测试序号：",i)
        if i == 700:
            print("跳过：",i)
        elif i ==9 or i == 10:
            title = Trade_query_batch_ccb_data.cases[i]["title"]
            case = Trade_query_batch_ccb_data.cases[i]["case"]
