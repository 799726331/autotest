...
from scripts1.datas.pay.data_ccb_004_trade_pay_batch_query import *
...
def data_list(test_host):
    """获取测试集"""
    data = Trade_pay_batch_query_ccb_data()
    return data
...
if __name__ == "__main__":
    ...
    for i in range(len(Trade_pay_batch_query_ccb_data.cases)):
        print("测试序号：",i)
        if i == 700:
            print("跳过：",i)
        elif i >=0 :
            title = Trade_pay_batch_query_ccb_data.cases[i]["title"]
            case = Trade_pay_batch_query_ccb_data.cases[i]["case"]
            expection = Trade_pay_batch_query_ccb_data.cases[i]["expection"]
